...
def packet_callback(packet):
    """Handle detected SMB network packets."""
    config = load_config()
    if not config:
        return
    ...
    if packet.haslayer(IP) and packet[IP].dst == target_ip:
        logging.info(f"SMB activity detected to {target_ip}")
...
def main():
    ...
    logging.info(f"Listening for SMB packets on {network_interface}...")

    sniff(iface=network_interface, filter=f"dst host {config['target_ip']} and port 445", prn=packet_callback, store=0)
